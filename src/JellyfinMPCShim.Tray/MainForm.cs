...
public partial class MainForm : Form, IJellyfinMessageHandler
{
    ...
    {
        // Previously this called _hostlLifetime.StopApplication(), which closed the
        // entire tray app on any dropped websocket connection (network blip, sleep/wake,
        // server restart, etc). Reconnection is handled by ConnectJellyfin's retry loop
        // instead, so the app stays running and recovers on its own.
        return Task.CompletedTask;
    }
